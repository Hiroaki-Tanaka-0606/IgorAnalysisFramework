...
//Function CalcMinOffset_k: calculate minimum offset, by which two adjacent slices (input[i][]) do not touch
Function/S IAFf_CalcMinOffset_k_Definition()
	return "3;0;0;1;Wave2D;Variable;Variable"
End
...
Function IAFf_CalcMinOffset_k(argumentList)
	...
	Variable i,j
	For(i=0;i<size1-1;i+=1)
		For(j=0;j<size2;j+=1)
			if(direction!=1)
				//input[i+1][] is above input[i][]
				Variable offset=input[i+1][j]-input[i][j]
				if(offset>minOffset)
					minOffset=offset
				Endif
			Else
				//input[i][] is above input[i+1][]
				offset=input[i+1][j]-input[i][j]
				if(offset>minOffset)
					minOffset=offset
				Endif
			Endif
...
//Function OffsetFigure_k: Add offset to each slice input[i][]
Function/S IAFf_OffsetFigure_k_Definition()
	return "5;0;0;0;0;1;Wave2D;Variable;Variable;Variable;Wave2D"
End
...
//Panel SliceFigure_E: make a figure of input[][i]s with offset
Function/S IAFp_SliceFigure_k_Definition()
	return "4;0;0;0;0;Variable;Variable;Variable;Wave2D"
End
...
Function IAFp_SliceFigure_k(argumentList, PanelName, PanelTitle)
	...
	sprintf command format,"DrawGraph_k",0,90,"Draw a Graph"
	Execute command
